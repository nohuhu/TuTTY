            ...
            KEY_SET_VALUE, &key) == ERROR_SUCCESS) {
                ...
                if (hk) {
                        if (RegSetValueEx(key, p, 0, 
                                REG_DWORD, (LPBYTE)&hk, sizeof(hk)) == ERROR_SUCCESS) {
                                RegCloseKey(key);
                                ret = TRUE;
                        };
                } else {
...
struct session *session_find_by_hotkey(struct session *root, int hotkey, int recurse) {
        ...
        for (i = 0; i < root->nchildren; i++) {
                if (recurse && root->children[i] && root->children[i]->type) {
                        tmp = session_find_by_hotkey(root->children[i], hotkey, recurse);
                        if (tmp)
                                return tmp;
                } else {
    ...
    for (i = 0; i < subkeys; i++) {
                if (RegEnumKeyEx(key, i, (LPTSTR)subkey, &size,
                        ...
                        if (RegOpenKeyEx(HKEY_CURRENT_USER, subname, 0,
                                ...
                                if (tmp2) {
                                        tmp->children = session_insert(tmp->children, &tmp->nchildren, tmp2);
                                        tmp2->parent = tmp;
                                };
                                RegCloseKey(key2);
                        } else {
...
static int new_session_record(char *name, int type) {
        ...
        if (!name ||
                RegCreateKeyEx(HKEY_CURRENT_USER, name, 0, NULL, REG_OPTION_NON_VOLATILE, 
        ...
        if (type) {
                ...
                value = 0;
                RegSetValueEx(key, ISEXPANDED, 0, REG_DWORD, (LPBYTE)&value, size);
        };
...
struct session *session_duplicate(struct session *s) {
        ...
        if (snew->nchildren) {
                snew->children = 
                        (struct session **)malloc(s->nchildren * sizeof(struct session *));
                for (i = 0; i < s->nchildren; i++)
...
static int copy_session_tree(char *from, char *to) {
        ...
        if ((err = RegOpenKeyEx(HKEY_CURRENT_USER, from, 0, KEY_READ, &key1)) != ERROR_SUCCESS)
                return FALSE;

        if ((err = RegCreateKeyEx(HKEY_CURRENT_USER, to, 0, NULL, REG_OPTION_NON_VOLATILE,
        ...
        if ((err = RegQueryInfoKey(key1, NULL, NULL, NULL, &subkeys, 
                NULL, NULL, &values, NULL, NULL, NULL, NULL)) != ERROR_SUCCESS) {
                RegCloseKey(key1);
                RegCloseKey(key2);
                return FALSE;
        };
        ...
        for (i = 0; i < (int)values; i++) {
                ...
                if (((err = RegEnumValue(key1, i, (LPSTR)name, &size, NULL,
                        ...
                        ((err = RegSetValueEx(key2, name, 0, subkeys, data, dsize)) != ERROR_SUCCESS)) {
                        free(data);
                        free(f);
                        free(t);
                        free(name);
