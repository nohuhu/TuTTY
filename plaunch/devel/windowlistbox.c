...
static int CALLBACK WindowListBoxProc(HWND hwnd, UINT msg,
    ...
    case WM_INITDIALOG:
		...
		if (!context_menu) {
			...
			AppendMenu(context_menu, MF_STRING | MF_GRAYED, IDC_WINDOWLISTBOX_BUTTON_HIDE, "&Hide");
			AppendMenu(context_menu, MF_STRING | MF_GRAYED, IDC_WINDOWLISTBOX_BUTTON_SHOW, "&Show");
			AppendMenu(context_menu, MF_SEPARATOR, 0, "");
			AppendMenu(context_menu, MF_STRING | MF_GRAYED, IDC_WINDOWLISTBOX_BUTTON_KILL, "&Kill");
		};
	...
	case WM_VKEYTOITEM:
		...
		case 'H':
			SendMessage(hwnd, WM_COMMAND, (WPARAM)IDC_WINDOWLISTBOX_BUTTON_HIDE, 0);
			return -2;
		case 'S':
			SendMessage(hwnd, WM_COMMAND, (WPARAM)IDC_WINDOWLISTBOX_BUTTON_SHOW, 0);
			return -2;
		case VK_DELETE:
			SendMessage(hwnd, WM_COMMAND, (WPARAM)IDC_WINDOWLISTBOX_BUTTON_KILL, 0);
			SetFocus(listbox);
			return -2;
		default:
			return -1;
...
void do_windowlistbox(void) {
	...
	DialogBox(config->hinst, MAKEINTRESOURCE(IDD_WINDOWLISTBOX), NULL, WindowListBoxProc);
