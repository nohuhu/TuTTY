...
#define CL_ATT513       0x4000	/* AT&T 513 terminal */
#define CL_OTHER        0x8000	/* Others, Xterm, linux, putty, dunno, etc */
...
#define TM_ATT513       (TM_VT100AVO|CL_ANSI|CL_ATT513)
					 ...
					 unsigned long *state))
{
	...
	if (thislen == prevlen &&
		...
		if (hdrsize == 0) {
		    ...
		    memmove(b->data + prevpos + 1, b->data + prevpos,
			    prevlen);
		    runpos = prevpos;
...
static void makeliteral_chr(struct buf *b, termchar * c,
			    unsigned long *state)
{
    /*
...
static void makeliteral_attr(struct buf *b, termchar * c,
			     unsigned long *state)
{
    /*
...
static void makeliteral_cc(struct buf *b, termchar * c,
			   unsigned long *state)
{
    /*
...
static void term_schedule_tblink(Terminal * term)
{
	...
	if (!term->tblink_pending)
	    term->next_tblink =
		schedule_timer(TBLINK_DELAY, term_timer, term);
...
static void term_schedule_cblink(Terminal * term)
{
	...
	if (!term->cblink_pending)
	    term->next_cblink =
		schedule_timer(CBLINK_DELAY, term_timer, term);
...
static void power_on(Terminal * term)
{
    ...
    term->alt_which = 0;
    memset(&term->last_printable, 0, sizeof(termchar));
...
void term_update(Terminal * term)
{
	...
	sys_cursor(term->frontend, term->curs.x,
		   term->curs.y - term->disptop);
	free_ctx(ctx);
    }
}
		    ...
		    void *frontend)
{
    ...
    /*
     * another quick&dirty hack: if the user wants to do some second-level
     * login (i.e. he's telnetting/sshing onto host connected to some
     * equipment via serial port or something alike, proceed the secondary
     * login script.
     * note that scripting is pretty basic: it will work only if there are
     * enough statements and those are correct.
     */
	...
	if (i) {
	    term->sec_proceed = TRUE;
	    split_into_argv(mycfg->secondaryscript, &term->sec_count,
			    &term->sec_buf, NULL);
 ...
 * on it! Blame xterm.)
 */
static void swap_screen(Terminal * term, int which, int reset,
			int keep_cur_pos)
{
    int t;
    tree234 *ttr;
	...
	term->screen = ttr;
	term->alt_sblines =
	    find_last_nonempty_line(term, term->alt_screen) + 1;
	t = term->curs.x;
 ...
 */
static void scroll(Terminal * term, int topline, int botline, int lines,
		   int sb)
{
    termline *line;
 ...
 * merge them.
 */
static void save_scroll(Terminal * term, int topline, int botline,
			int lines)
{
 ...
 */
static void scroll_display(Terminal * term, int topline, int botline,
			   int lines)
{
		       ...
		       int line_only, int from_begin, int to_end)
{
	...
	start.y = 0;
	start.x = 0;
	/* at&t 513 erases within the scroll margins */
	if ((has_compat(ATT513)) && (!from_begin)) {
	    end.y = term->marg_b + 1;
	} else {
	    end.y = term->rows;
	}
...
static void toggle_mode(Terminal * term, int mode, int query, int state)
{
	...
	case 3:		/* DECCOLM: 80/132 columns */
		...
		request_resize(term->frontend, state ? 132 : 80,
			       term->rows);
	...
	case 1047:		/* alternate screen */
	    compatibility(OTHER);
	    deselect(term);
	    swap_screen(term, term->cfg.no_alt_screen ? 0 : state, TRUE,
			TRUE);
	    term->disptop = 0;
	    break;
...
static void do_osc(Terminal * term)
{
	...
	while (term->osc_strlen--)
	    term->wordness[(unsigned char)
			   term->osc_string[term->osc_strlen]] =
		term->esc_args[0];
    } else {
...
static void term_out(Terminal * term)
{
	...
	if ((c & ~0x1F) == 0 && term->termstate < DO_CTRLS) {
		 ...
		 * ctrls are stripped above */
		{
			...
			if (term->curs.y == term->marg_b)
			    scroll(term, term->marg_t, term->marg_b, 1,
				   TRUE);
			else if (term->curs.y < term->rows - 1)
			    term->curs.y++;
			term->curs.x = 0;
			...
			logtraffic(term->logctx, (unsigned char) c,
				   LGTYP_ASCII);
		    /*
		     * okay, so we have printable character, let's match it
		     * against secondary script.
		     */
		    if (term->sec_proceed && term->cfg.secondary) {
			char *p = term->sec_buf[term->sec_item];
			Ldisc ld = term->ldisc;
...
//                                ldisc_send(term->ldisc, "\r", 1, 0);
				...
				if (term->sec_item < term->sec_count) {
				    term->sec_len =
					strlen(term->
					       sec_buf[term->sec_item]);
				    term->sec_pos = 0;
				} else {	/* end of script */
				    term->sec_proceed = FALSE;
				};
			    };
			} else if (term->sec_pos) {
			    term->sec_pos = 0;
			};
		    };
			 ...
			 */
			check_boundary(term, term->curs.x, term->curs.y);
			check_boundary(term, term->curs.x + 2,
				       term->curs.y);
			if (term->curs.x == term->cols - 1) {
			    ...
			    check_boundary(term, term->curs.x,
					   term->curs.y);
			    check_boundary(term, term->curs.x + 2,
					   term->curs.y);
			}
		    ...
		    case 1:
			check_boundary(term, term->curs.x, term->curs.y);
			check_boundary(term, term->curs.x + 1,
				       term->curs.y);
			...
			/* 513 terminal doesn't support unicode anyway,
			   so I made this only for normal characters */
			term->last_printable.chr = c;
			term->last_printable.attr = term->curr_attr;
			break;
		    case 0:
			if (term->curs.x > 0) {
			    int x = term->curs.x - 1;
		...
		case 'S':	/* afaik ^SB is "thumbprint" of at&t 513 terminal */
		    probably_513 = TRUE;
		    term->termstate = SEEN_ESC;
		    break;
		case 'B':	/* ... and if we see it, we assume we have 513 :) */
		    if (probably_513)
			term->compatibility_level = TM_ATT513;
		    break;
		case '[':	/* enter CSI mode */
		...
		if (isdigit(c)) {
			...
			break;
		    case 'b':	/* repeat last character N times */
			{
			    int i;

			    compatibility(ATT513);
				...
				if (term->selstate != NO_SELECTION) {
				    pos cursplus = term->curs;
				    incpos(cursplus);
				    check_selection(term, term->curs,
						    cursplus);
				}
				if (((c & CSET_MASK) == CSET_ASCII ||
				     (c & CSET_MASK) == 0) && term->logctx)
				    logtraffic(term->logctx,
					       (unsigned char) c,
					       LGTYP_ASCII);
...
//                              fix_cpos;
			...
			if (term->esc_nargs <= 1
			    ...
			    case 14:
				if (term->ldisc) {
				    get_window_pixels(term->frontend, &x,
						      &y);
				    len =
					sprintf(buf, "\033[4;%d;%dt", x,
						y);
				    ldisc_send(term->ldisc, buf, len, 0);
				}
				break;
				...
				if (term->ldisc &&
				    ...
				    p = get_window_title(term->frontend,
							 TRUE);
				    len = strlen(p);
	    ...
	    case SEEN_OSC_P:
		{
			...
			palette_set(term->frontend, term->osc_string[0],
				    term->osc_string[1] * 16 +
				    term->osc_string[2],
				    term->osc_string[3] * 16 +
				    term->osc_string[4],
				    term->osc_string[5] * 16 +
				    term->osc_string[6]);
			term_invalidate(term);
			term->termstate = TOPLEVEL;
		    }
		...
		case 'L':
		    /* compatibility(ATARI) */
		    if (term->curs.y <= term->marg_b)
			scroll(term, term->curs.y, term->marg_b, -1,
			       FALSE);
		    break;
		case 'M':
		    /* compatibility(ATARI) */
			       ...
			       termchar * lbefore, int width)
{
    ...
    for (i = 0; i < width; i++)
	if (!termchars_equal
	    (term->pre_bidi_cache[line].chars + i, lbefore + i))
...
static void term_bidi_cache_store(Terminal * term, int line,
				  termchar * lbefore, termchar * lafter,
...
static void do_paint(Terminal * term, Context ctx, int may_optimise)
{
	...
	if ((lchars = term_bidi_line(term, ldata, our_curs_y)) != NULL) {
	    our_curs_x =
		term->post_bidi_cache[our_curs_y].forward[our_curs_x];
	} else
	    lchars = ldata->chars;
	...
	(term->curstype != cursor ||
	 term->dispcursy != our_curs_y || term->dispcursx != our_curs_x)) {
...
void term_scroll(Terminal * term, int rel, int where)
{
    ...
    term->disptop =
	(rel < 0 ? 0 : rel > 0 ? sbtop : term->disptop) + where;
		   ...
		   int desel)
{
	...
	while (poslt(top, bottom) && poslt(top, nlpos)) {
		...
		if (DIRECT_FONT(uc)) {
			...
			int rv;
			if (is_dbcs_leadbyte
			    (term->ucsdata->font_codepage, (BYTE) c)) {
			    buf[0] = c;
			    buf[1] =
				(char) (0xFF & ldata->chars[top.x + 1].
					chr);
			    rv = mb_to_wc(term->ucsdata->font_codepage, 0,
					  buf, 2, wbuf, 4);
			    top.x++;
			} else {
			    buf[0] = c;
...
static pos sel_spread_half(Terminal * term, pos p, int dir)
{
	...
	if (dir == +1) {
		...
		if (p.x < maxcols - 1) {
		    if (wordtype(term, UCSGET(ldata->chars, p.x + 1)) ==
			wvalue)
			p.x++;
		    else
			break;
		...
		if (p.x > 0) {
			...
			if (wordtype
			    (term, UCSGET(ldata2->chars, maxcols - 1))
			    == wvalue) {
			    p.x = maxcols - 1;
			    p.y--;
			    unlineptr(ldata);
...
void term_do_paste(Terminal * term)
{
	...
	if (term->paste_len < 256) {
	    if (term->ldisc)
		luni_send(term->ldisc, term->paste_buffer, term->paste_len,
			  0);
	    if (term->paste_buffer)
		sfree(term->paste_buffer);
	    term->paste_buffer = 0;
	      ...
	      unsigned int modifiers, unsigned int flags)
{
	...
	*p++ = 0x1b;
	...
	if (term->vt52_mode)
	    ...
	    int app_flg = (term->app_cursor_keys
			   && !term->cfg.no_applic_c);
...
{
    term->selstate = NO_SELECTION;
    term->selstart.x = term->selstart.y = term->selend.x = term->selend.y =
	0;
}
