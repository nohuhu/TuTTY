...
void scp_source_setup(char *target, int shouldbedir)
{
	...
	if (!fxp_init()) {
	    tell_user(stderr, "unable to initialise SFTP: %s",
		      fxp_error());
	    errs++;
	    return;
	}
...
int scp_send_finish(void)
{
	...
	if (scp_has_times) {
	    ...
	    sftp_register(req =
			  fxp_fsetstat_send(scp_sftp_filehandle, attrs));
	    rreq = sftp_find_request(pktin = sftp_recv());
	    assert(rreq == req);
...
int scp_get_sink_action(struct scp_sink_action *act)
{
	...
	while (!done) {
	    ...
	    case 'D':
		act->action =
		    (action == 'C' ? SCP_SINK_FILE : SCP_SINK_DIR);
		break;
	    default:
...
int scp_accept_filexfer(void)
{
	...
	sftp_register(req =
		      fxp_open_send(scp_sftp_currentname, SSH_FXF_READ));
	rreq = sftp_find_request(pktin = sftp_recv());
	assert(rreq == req);
	scp_sftp_filehandle = fxp_open_recv(pktin, rreq);
...
int scp_recv_filedata(char *data, int len)
{
	...
	if (ret < 0) {
	    tell_user(stderr, "pscp: error while reading: %s",
		      fxp_error());
	    errs++;
	    return -1;
	}
