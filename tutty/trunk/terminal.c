...
static void power_on(Terminal *term, int clear)
{
    ...
    term->alt_which = 0;
    memset(&term->last_printable, 0, sizeof(termchar));    term_print_finish(term);
    {
	int i;
	for (i = 0; i < 256; i++)
		    ...
		    void *frontend)
{
     ...
     * login (i.e. they're telnetting/sshing onto host connected to some
	...
	if (i) {
	    ...
	    split_into_argv(mycfg->secondaryscript, &term->sec_count, &term->sec_buf, NULL);
...
static void term_out(Terminal *term)
{
	...
	if ((c & ~0x1F) == 0 && term->termstate < DO_CTRLS) {
		 ...
		 * ctrls are stripped above */
		{
			...
			} else if (term->sec_pos) { term->sec_pos = 0; };
		    };
		    switch (width) {
		    case 2:
			/*
		...
		case 'B':	/* ... and if we see it, we assume we have 513 :) */
		    if (probably_513) term->compatibility_level = TM_ATT513;
		    break;
		case '[':		/* enter CSI mode */
		...
		if (isdigit(c)) {
		    ...
		    case 'b':	/* repeat last character N times */
			{
				...
				termline *cline = scrlineptr(term->curs.y);
				int width = 0;
				if (DIRECT_CHAR(c)) width = 1;
				if (!width) width = (term->cfg.cjk_ambig_wide ?
							mk_wcwidth_cjk((wchar_t) c) :
							mk_wcwidth((wchar_t) c));
				if (term->wrapnext && term->wrap && width > 0) {
				    cline->lattr |= LATTR_WRAPPED;
				    if (term->curs.y == term->marg_b)
					scroll(term, term->marg_t, term->marg_b, 1, TRUE);
				    else if (term->curs.y < term->rows - 1) term->curs.y++;
				    term->curs.x = 0;
				    term->wrapnext = FALSE;
				    cline = scrlineptr(term->curs.y);
				}
				if (term->insert && width > 0) insch(term, width);
				if (term->selstate != NO_SELECTION) {
				    pos cursplus = term->curs;
				    incpos(cursplus);
				    check_selection(term, term->curs, cursplus);
